...
function ImageFlicker({ items, interval = 5000, delay = 0 }: { items: { src: string, projectId: string }[], interval?: number, delay?: number }) {
  ...
  return (
    <Link to={`/projects/${items[index].projectId}`} className="block w-full h-full relative overflow-hidden group">
...
function TextFlicker({ projects, interval = 8000, delay = 0, color = "#7F4F3F" }: { projects: Project[], interval?: number, delay?: number, color?: string }) {
  const [index, setIndex] = useState(0);
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        ...
        >
          <h2 className="h2 text-center w-full group-hover:opacity-60 transition-opacity" style={{ color }}>
            {getLocalized(current?.title)}
          </h2>
          <p className="metadata-token text-center w-full" style={{ color: color + 'CC' }}>
  ...
  const flickerGroups = useMemo(() => {
    ...
    latestProjects.forEach(p => {
      if (p.heroImage && !p.heroImage.toLowerCase().includes('color')) {
        pool.push({ src: p.heroImage, projectId: p.id });
      }
      (p.images || []).forEach(img => {
        if (img && !img.toLowerCase().includes('color')) {
          pool.push({ src: img, projectId: p.id });
        }
      });
    });
    ...
    <div className="flex flex-col bg-bg-primary overflow-x-hidden min-h-screen">
      ...
      <section className="page-padding section-padding">
        <div className="max-w-[2810px] mx-auto grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-0">
          ...
          {/* Block 1: Main Feature (Flickering Projects) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <ImageFlicker 
              items={flickerGroups.main.length > 0 ? flickerGroups.main : [
                { src: latestProjects[0]?.heroImage || "https://picsum.photos/seed/archi-1/1200/1200", projectId: latestProjects[0]?.id || '' }
              ]} 
              interval={7000}
            />
          </motion.div>

          {/* Block 3: Dynamic Image 2 (Animated) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <ImageFlicker 
              items={flickerGroups.mid.length > 0 ? flickerGroups.mid : [
                { src: latestProjects[1]?.heroImage || "https://picsum.photos/seed/archi-2/600/600", projectId: latestProjects[1]?.id || '' }
              ]} 
              interval={5500}
              delay={1000}
            />
          ...
          >
            <h3 className="h3 !font-bold text-center !leading-[1.1] !uppercase tracking-tighter text-[#919A8F] !text-[16px] md:!text-[18px] lg:!text-[20px]" dangerouslySetInnerHTML={{ __html: t('home_experience_title').replace(/\s+/g, '<br />') }}>
            </h3>
          </motion.div>

          {/* Block 4b: Extra Image to fill gap (Animated) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          {/* Block 5: Dynamic Image 3 (Animated) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <ImageFlicker 
              items={flickerGroups.small.length > 0 ? flickerGroups.small : [
                { src: latestProjects[2]?.heroImage || "https://picsum.photos/seed/archi-3/600/600", projectId: latestProjects[2]?.id || '' }
              ]} 
              interval={6000}
              delay={2000}
            />
          ...
          {/* Block 6: Dynamic Image 4 (Animated) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <h3 className="h3 !font-bold text-center !leading-[1.1] !uppercase tracking-tighter text-[#B2B369] !text-[16px] md:!text-[18px] lg:!text-[20px]" dangerouslySetInnerHTML={{ __html: t('home_ambassador_title').replace(/\s+/g, '<br />') }}>
            </h3>
          </motion.div>

          {/* Block 8: Dynamic Image 5 (Animated) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          {/* Block 9: Territory/Projects Flicker */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          {/* Block 10: Dynamic Image 6 (Animated) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          {/* Block 11: Dynamic Image 7 (Animated) */}
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
